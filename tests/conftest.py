...
FOODGRAM_DOMAIN_KEY = 'foodgram_domain'
DOCKERFILE_NAME = 'Dockerfile'
DOCKERHUB_USERNAME_KEY = 'dockerhub_username'
WORKFLOW_FILE = 'infra/foodgram_workflow.yml'
DOCKER_COMPOSE_PROD_FILE_NAME = 'infra/docker-compose.production.yml'
...
def expected_deploy_info_file_content() -> dict[str, str]:
    return {
        ...
        FOODGRAM_DOMAIN_KEY: 'ссылка для доступа к проекту `Foodgran`',
        'dockerhub_username': 'ваше имя пользователя на Docker Hub',
    }


@pytest.fixture(params=(FOODGRAM_DOMAIN_KEY,))
...
def link_keys() -> tuple[str, str]:
    return (FOODGRAM_DOMAIN_KEY,)


@pytest.fixture(scope='session')
def foodgram_link_key() -> str:
    return FOODGRAM_DOMAIN_KEY
